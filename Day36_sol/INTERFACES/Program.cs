namespace INTERFACES
{
        ...
        internal class Program
        {
            ...
            Console.WriteLine($"\nTotal films: {library.GetTotalFilmCount()}");
                ...
                Console.WriteLine();
                var firstfilm = library.GetFilms().OrderBy(x => x.Year).Take(3);
                Console.WriteLine("-----The first three flim ---- " + "\n"+ string.Join('\n', firstfilm));
                
                var firstflim200 = library.GetFilms().OrderBy(x => x.Year).TakeWhile(X=>X.Year<2000);
                Console.WriteLine("-----The flim before 2000 ---- " + "\n" + string.Join('\n', firstflim200));

            int currentyear = DateTime.Now.Year;

            var filmprojection = library.GetFilms().Select(x => new
            {
                x.Title,
                x.Director,
                x.Year,
            ...
            Console.WriteLine("-----The age of flim  ---- " + "\n" + string.Join('\n', filmprojection ));

            var momneto = library.GetFilms().SingleOrDefault(x => x.Title.Equals("Inception", StringComparison.OrdinalIgnoreCase));
            if (momneto != null)
            {
                Console.WriteLine(momneto);
            }
            else
            {
                Console.WriteLine("value can be found");
            }
